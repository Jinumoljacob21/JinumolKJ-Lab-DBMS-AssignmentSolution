...
INNER JOIN  orders as O on  C.CUS_ID=O.CUS_ID where O.ORD_AMOUNT>=3000 
group by C.CUS_GENDER;

-- 4)	Display all the orders along with the product name ordered by a customer having Customer_Id=2.

SELECT  O.ORD_ID,O.ORD_AMOUNT ,O.ORD_DATE,O.CUS_ID,O.PROD_ID,P.PRO_NAME 
FROM orders AS O, PRODUCT_DETAILS AS PD,PRODUCT  AS P 
WHERE O.PROD_ID = PD.PROD_ID AND PD.PRO_ID = P.PRO_ID AND O.CUS_ID = 2;

-- 5)	Display the Supplier details who can supply more than one product

select s.SUPP_ID,s.SUPP_NAME,s.SUPP_CITY,s.SUPP_PHONE 
from supplier as s ,product_details as pd 
where s.SUPP_ID=pd.SUPP_ID having count(pd.SUPP_ID)>1;


-- 6)	Find the category of the product whose order amount is minimum

SELECT C.CAT_ID,C.CAT_NAME FROM orders AS O 
 INNER JOIN product_details AS PD ON O.PROD_ID=PD.PROD_ID 
 INNER JOIN product AS P ON PD.PRO_ID=P.PRO_ID 
 INNER JOIN category AS C ON P.CAT_ID =C.CAT_ID having  MIN(O.ORD_AMOUNT);

-- 7)	Display the Id and Name of the Product ordered after “2021-10-05”.

select product. PRO_ID,product. PRO_NAME FROM product,orders,product_details 
where product.PRO_ID=product_details.PRO_ID and product_details.PROD_ID=orders.PROD_ID 
...
END AS Verdict FROM rating as R inner join supplier as S ON S.SUPP_ID=R.SUPP_ID;
